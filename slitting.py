...
import generate_results_tables
...
STATUS_MAP = {
    ...
    GRB.UNBOUNDED: "Modelo ilimitado",
    GRB.CUTOFF: "Objetivo atingiu limite de cutoff",
    GRB.ITERATION_LIMIT: "Limite de iterações atingido",
    GRB.NODE_LIMIT: "Limite de nós de branch-and-bound atingido",
    GRB.SOLUTION_LIMIT: "Número máximo de soluções encontrado",
    GRB.INTERRUPTED: "Interrompido por usuário",
    GRB.TIME_LIMIT: "Tempo limite atingido",
    GRB.SUBOPTIMAL: "Solução encontrada, mas pode não ser ótima",
    GRB.NUMERIC: "Problemas numéricos graves",
    # GRB.LICENSE_LIMIT: "Limite da licença atingido",
    GRB.USER_OBJ_LIMIT: "Otimização interrompida por limite definido pelo usuário no objetivo"
}
    ...
    for reusable_coef_y in {2, 4}:
        for reusable_coef_v in {2, 4}:
            ...
            objective_coefficients = {
                'use_v': 1,
                'edge_trim': scrap_coef,
                'retail_y': reusable_coef_y,
                'scrap_y': scrap_coef,
                'retail_v': reusable_coef_v,
                'scrap_v': scrap_coef,
                'endloss': scrap_coef,
                'rolled_leftover': reusable_coef_v,
                'scrap_processed': scrap_coef
            }

            for objective_type in objective_types:
                print("=" * 50)
                print(f"Preparando objetivo {objective_type}")
                print("=" * 50)
                ...
                for data_folder in dirs:
                    ...
                    for period in ['period_1', 'period_2', 'period_3', 'period_4', 'period_5']:
                        print(f"{period} iniciado em {dt.datetime.now().strftime("%H:%M:%S")}", end="")

                        log_path = Path(f'output/{data_folder}/{period}/slitting.log')
                        ...
                        parts = pd.read_csv(
                            ...
                            dtype={'Code': 'str', 'Width': 'float', 'Demand': 'float', 'BeginLoss': 'float',
                                   'EndLoss': 'float'}
                        )
                        ...
                        # Length (m)
                        parts['Length'] = ((parts['Demand'] / parameters['Density']) / (
                                parameters['Thickness'] * parts['Width']))
                        ...
                        # Length (m)
                        coils['Length'] = ((coils['Weight'] / parameters['Density']) / (
                                parameters['Thickness'] * coils['Width']))
                        # PesoM2 (kg/m2)
                        coils['WeightPerM2'] = (parameters['Density'] * parameters['Thickness'])
                        # Maximo de tiras
                        coils['MaxNbStrips'] = np.minimum(np.ceil((coils['Width']) / min_strip_width),
                                                          parameters['NumKnives'] - 1).astype(int)
                        # Value (R$/kg -> R$)
                        ...
                        dtypes = {'Id': 'str', 'Width': 'float', 'Value': 'float', 'Weight': 'float',
                                  'Processed': 'int'}
                        try:
                            retails = pd.read_csv(f'data/{data_folder}/{period}/Retails.csv', usecols=cols,
                                                  dtype=dtypes)
                        ...
                        # Length (m)
                        retails['Length'] = (
                                (retails['Weight'] / parameters['Density']) / (
                                parameters['Thickness'] * retails['Width']))
                        # PesoM2 (kg/m2)
                        retails['WeightPerM2'] = (parameters['Density'] * parameters['Thickness'])
                        # Maximo de tiras
                        retails['MaxNbStrips'] = np.minimum(np.ceil((retails['Width']) / min_strip_width),
                                                            parameters['NumKnives'] - 1).astype(int)
                        # Value (R$/kg -> R$)
                        ...
                        gurobi_log_path = Path(f'output/{data_folder}/{period}/gurobi.log')
                        ...
                        # (new)
                        model.addConstrs(
                            (vr[p, c, s] >= parameters['MinRetailLength'] * beta[p, c, s] for p in parts_codes for
                             (c, s)
                             in
                        ...
                        for coil in coils.itertuples():
                            c = coil.Id
                            model.addConstr(
                                e[c] <= 2 * parameters['MinEdgeTrim'] * coil.WeightPerM2 * coil.Length * alpha.get(
                                    (c, 2), 0),
                                name=f'edgeTrim2[{c}]'
                            )
                                ...
                                for s2 in slits[:i_s1 + 1]:
                                    for i_p1, p1 in enumerate(parts_codes):
                                        ...
                                        if len(p2_list) == 0:
                                            continue
                                        model.addConstr(
                                            mu[p1, c, s1] + mu.sum(p2_list, c, s2) <= 1,
                                            name=f"valid[{p1},{c},{s1},{s2}]"
                                        )
                        ...
                        if model.SolCount > 0:
                            ...
                            output_retails_path = (
                                f"data/{data_folder}/{next_period}/Retails.csv"
                            )
                        ...
                        json_output_path = (
                            f"output/{data_folder}/{period}/cutting_plan.json"
                        )
                        ...
                        status = STATUS_MAP.get(model.Status)
                        model.dispose()
                        gp.disposeDefaultEnv()

                        print(f" e concluído em {dt.datetime.now().strftime("%H:%M:%S")}. {status}")
                ...
                for src in sources:
                    src_path = Path(src)
                    target = destination / src_path.name
                    print(f"Copiando {src_path} -> {target}")
