...
for objective_type in objective_types:
    ...
    for data_folder in dirs:
        ...
        for period in ['period_1', 'period_2', 'period_3', 'period_4', 'period_5']:
            ...
            try:
                retails = pd.read_csv(f'data/{data_folder}/{period}/Retails.csv', usecols=cols, dtype=dtypes)
            ...
            # Length (m)
            retails['Length'] = (
                        (retails['Weight'] / parameters['Density']) / (parameters['Thickness'] * retails['Width']))
            # PesoM2 (kg/m2)
    ...
    destination = Path(f"objectives/{objective_type}")
